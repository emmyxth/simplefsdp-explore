...
def fsdp_main(rank, world_size, args):
    try:
        ...
        world_mesh = init_device_mesh(device_type="cuda", mesh_shape=(2,))
        ...
        print(f'FSDP model: {fsdp_model}')

        # Torch Compile Set Up
        optimized_model = torch.compile(fsdp_model)

        print(f'Optimized model: {optimized_model}')

        optimizer = optim.Adadelta(optimized_model.parameters(), lr=args.lr)

        scheduler = StepLR(optimizer, step_size=1, gamma=args.gamma)

        # Training / Testing Epoch Start
        init_start_event.record()
        for epoch in range(1, args.epochs + 1):
            train(args, optimized_model, rank, world_size,
                  train_loader, optimizer, epoch, sampler=sampler1)
            test(optimized_model, rank, world_size, test_loader)
            scheduler.step()
        ...
        if args.optimized_model:
            ...
            dist.barrier()
            states = optimized_model.state_dict()
            if rank == 0:
